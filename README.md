...
\- `parse\_feed(url)` - Fetch and parse RSS feed

\- `display\_articles(articles)` - Format and display articles

\- `fetch\_category(category)` - Load news by category

\- `search\_news(query)` - Search news with URL encoding

\- `open\_article(number)` - Open in default browser
